...
public class AutoCommandFactory {
    ...
    public AutoRoutine justShoot() {
        ...
        routine.active().onTrue(moveToStart);
        moveToStart.done().onTrue(CommandFactory.justShoot(swerve.state, () -> {
            return AllianceFlipUtil.apply(FieldConstants.Hub.centerHub);
        }, swerve, shooter, indexer, adjustableHood, magazine, () -> 0, () -> 0, () -> true));
        return routine;
    }
    ...
    private Command wilsonTestSide(boolean left) {
        ...
        return Commands
            .sequence(sweep(left, true, Constants.Auto.wilsonTestX, driveSpeed),
                CommandFactory
                    .justShoot(swerve.state,
                        () -> AllianceFlipUtil.apply(FieldConstants.Hub.centerHub), swerve, shooter,
                        indexer, adjustableHood, magazine, () -> 0.0, () -> left ? turretFudge
                            : -turretFudge,
                        () -> false)
                ...
                Commands
                    .sequence(adjustableHood.setGoal(Rotations.of(0)),
                        ...
                        CommandFactory
                            .justShoot(swerve.state,
                                () -> AllianceFlipUtil.apply(FieldConstants.Hub.centerHub), swerve,
                                shooter, indexer, adjustableHood, magazine, () -> 0.0,
                                () -> left ? turretFudge : -turretFudge, () -> false)
                            .alongWith(intake.jerkIntake()).withTimeout(shootingTime),
                        adjustableHood.setGoal(Rotations.of(0)),
                        sweep(left, false, 8.076, driveSpeed),
                        CommandFactory
                            .justShoot(swerve.state,
                                () -> AllianceFlipUtil.apply(FieldConstants.Hub.centerHub), swerve,
                                shooter, indexer, adjustableHood, magazine, () -> 0.0,
    ...
    private Command sweep(boolean left, boolean isFirst, double xMeters, double driveSpeed) {
        return Commands
            ...
            .deadlineFor(new InstantCommand(() -> swerve.turnToRotation(() -> 0.0)));
    }
}
